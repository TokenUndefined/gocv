...
//
func CircleEx(img *Mat, center image.Point, radius int, c color.RGBA, thickness int, linetype int) {
	pc := C.struct_Point{
		x: C.int(center.X),
		y: C.int(center.Y),
	}
	...
	C.CircleEx(img.p, pc, C.int(radius), sColor, C.int(thickness), C.int(linetype))
}
...
// https://docs.opencv.org/master/d6/d6e/group__imgproc__draw.html#ga28b2267d35786f5f890ca167236cbc69
//
func EllipseEx(img *Mat, center, axes image.Point, angle, startAngle, endAngle float64, c color.RGBA, thickness int, linetype int) {
	pc := C.struct_Point{
		x: C.int(center.X),
		y: C.int(center.Y),
	}
	...
	C.EllipseEx(img.p, pc, pa, C.double(angle), C.double(startAngle), C.double(endAngle), sColor, C.int(thickness), C.int(linetype))
}
...
// https://docs.opencv.org/master/d6/d6e/group__imgproc__draw.html#ga7078a9fae8c7e7d13d24dac2520ae4a2
//
func LineEx(img *Mat, pt1 image.Point, pt2 image.Point, c color.RGBA, thickness int, linetype int) {
	sp1 := C.struct_Point{
		x: C.int(pt1.X),
		y: C.int(pt1.Y),
	}
	...
	C.LineEx(img.p, sp1, sp2, sColor, C.int(thickness), C.int(linetype))
}
...
func FillPolyEx(img *Mat, pts [][]image.Point, c color.RGBA, linetype int) {
	...
	C.FillPolyEx(img.p, cPoints, sColor, C.int(linetype))
}
