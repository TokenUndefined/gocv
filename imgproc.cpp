...
void CircleEx(Mat img, Point center, int radius, Scalar color, int thickness, int linetype) {
    cv::Point p1(center.x, center.y);
    cv::Scalar c = cv::Scalar(color.val1, color.val2, color.val3, color.val4);

    cv::circle(*img, p1, radius, c, thickness, linetype);
}
...
void EllipseEx(Mat img, Point center, Point axes, double angle, double
             startAngle, double endAngle, Scalar color, int thickness, int linetype) {
    cv::Point p1(center.x, center.y);
    cv::Point p2(axes.x, axes.y);
    cv::Scalar c = cv::Scalar(color.val1, color.val2, color.val3, color.val4);
...
void LineEx(Mat img, Point pt1, Point pt2, Scalar color, int thickness, int linetype) {
    cv::Point p1(pt1.x, pt1.y);
    cv::Point p2(pt2.x, pt2.y);
    cv::Scalar c = cv::Scalar(color.val1, color.val2, color.val3, color.val4);

    cv::line(*img, p1, p2, c, thickness, linetype);
}
...
void FillPolyEx(Mat img, Contours points, Scalar color, int lineType) {
    std::vector<std::vector<cv::Point> > pts;
    ...
    cv::fillPoly(*img, pts, c, lineType);
}
